...
## 5. Carga el módulo loop, obtén información de qué es y para qué sirve. Lista el contenido de /sys/modules/loop/parameters y configura el equipo para que se puedan cargar como máximo 12 dispositvos loop la próxima vez que se arranque.
...
`Pop!_OS`, que usa `systemd-boot`, la configuración del arranque se maneja de manera diferente a GRUB. Para establecer el número máximo de dispositivos loop, necesitas editar el archivo de configuración del `kernel`.

* Abre el archivo de configuración del kernel para edición:
...
Tras modificar la configuración del kernel, debes actualizar systemd-boot para aplicar los cambios:
...
Voy a explicarte paso a paso cómo realizar cada uno de estos ejercicios en Pop!_OS, manteniendo tu estilo claro y técnico:
...
:::tip Magic SysRq
Magic SysRq es una funcionalidad del kernel de Linux que proporciona un método directo de comunicación con el kernel, permitiendo ejecutar comandos de bajo nivel independientemente del estado del sistema. Es una herramienta valiosa para situaciones de recuperación del sistema y diagnóstico.
...
- **Comandos Comunes**: Incluyen reiniciar el sistema (B), matar todos los procesos excepto el proceso de inicio (I), y desmontar sistemas de archivos de forma segura (U).
:::

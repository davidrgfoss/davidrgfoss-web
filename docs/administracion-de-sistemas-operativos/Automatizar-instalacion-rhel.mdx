...
part pv.111 --fstype="lvmpv" --ondisk=vda --size=91135
...
logvol / --fstype="xfs" --size=44207 --name=root --vgname=rhel
...
* Luego en la misma ubicación de las líneas anteriores añadimos lo siguiente:
 ...
 label linux
  ...
  menu default
  kernel vmlinuz
  append initrd=initrd.img inst.stage2=hd:LABEL=RHEL-9-0-0-BaseOS-x86_64 inst.ks=cdrom:/ks.cfg
...
* Luego en el mismo sitio, escribimos las siguientes líneas:
 ...
 menuentry 'Install Automate RHEL9' --class fedora --class gnu-linux --class gnu --class os {
    linuxefi /images/pxeboot/vmlinuz inst.stage2=hd:LABEL=RHEL-9-0-0-BaseOS-x86_64 inst.ks=cdrom:/ks.cfg
    initrdefi /images/pxeboot/initrd.img
}
 ```
...
* Copiamos el fichero “ks.cfg” dentro del directorio “/tmp/rhel9” y generamos la “ISO” a partir de este directorio con el siguiente comando:
 ```jsx
 mkisofs -o /tmp/rhel9A.iso -b isolinux/isolinux.bin -J -R -l -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e images/efiboot.img -no-emul-boot -graft-points -V "RHEL-9-0-0-BaseOS-x86_64" .
 ```
:::danger Precaupción
...
# Especificamos el servidor que contendra todos los datos de la imagen que vamos a instalar.
